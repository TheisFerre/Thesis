...
open_file = open("~/Thesis/data/processed/202106-citibike-tripdata.pkl", "rb")
...
def objective(trial):
    ...
    train_dataset, test_dataset = Dataset.train_test_split(dataset, num_history=8, ratio=0.8)
    ...
    weight_decay = trial.suggest_float("weight_decay", 0, 1e-1, log=True)
    learning_rate = trial.suggest_float("lr", 1e-6, 1e-2, log=True)
    epochs = trial.suggest("epochs", 100, 400)
...
if __name__ == "__main__":
    ...
    trial = study.best_trial
    opt_param_dict = {key: value for key, value in trial.params.items()}
    params_dict = {
    ...
    with open(f"edgeconv_hyperopt_{end_time_str}/settings.json", "w") as outfile:
